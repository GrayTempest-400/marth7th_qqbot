...
from .marth7th_ai.qqbot_speech import tts
...
class NewBing:
    ...
    async def bing_handle(
        ...
        try:  # 尝试发送回复
            ...
            if tts(rep_message):
                await matcher.send(
                    MessageSegment.record(
                        "marth7th_ai/results/mar7th_tts.wav"
                    ),
                    reply_message=True,
                )
            else:
                await matcher.send(
                    MessageSegment.text(tts(rep_message+"tts运行错误，请检测配置")),reply_message=True,)
            if max_conversation <= current_conversation:
                tts("达到对话上限, 正帮你重置会话")
                await matcher.send(
                    MessageSegment.record("marth7th_ai/results/mar7th_tts.wav"), reply_message=True
                )
                try:
                    await utils.newbing_new_chat(event=event, matcher=matcher)
                except Exception:
            ...
            try:
                await matcher.send(
                    MessageSegment.text(f"语音消息可能被风控了\n错误信息:{repr(e)}\n这里咱尝试把文字写在图片上发送了")
