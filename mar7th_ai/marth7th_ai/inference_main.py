...
def main():
    ...
    # 一定要设置的部分
    parser.add_argument('-m', '--model_path', type=str, default="mar7th_G_40000.pth", help='模型路径')
    parser.add_argument('-c', '--config_path', type=str, default="mar7th.json", help='配置文件路径')
...
def modelAnalysis(model_path,config_path,cluster_model_path,device,enhance,diff_model_path,diff_config_path,only_diffusion,use_spk_mix,local_model_enabled,local_model_selection):
    ...
    try:
        ...
        if (local_model_enabled):
            # local path
            model_path = "mar7th_G_40000.pth"
            config_path = "mar7th.json"
        else:
            # upload from webpage
...
def vc_fn2(_text, _lang, _gender, _rate, _volume, sid, output_format, vc_transform,
           auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,
           lgr_num,f0_predictor,enhancer_adaptive_key,cr_threshold, k_step,use_spk_mix,
    ...
    try:
        ...
        output_file_path = vc_infer(output_format, sid, input_audio,
                                    "tts", vc_transform, auto_f0, cluster_ratio,
                                    slice_db, noise_scale, pad_seconds, cl_num, lg_num,
                                    lgr_num, f0_predictor, enhancer_adaptive_key, cr_threshold,
...
if __name__ == '__main__':
    output_format = {"value": "wav"}
    sid = "mar7th"
    input_audio = "tts.wav"
    vc_transform = {"value": 0}
    auto_f0 = {"value": True}
    cluster_ratio = {"value": 0}
    slice_db = {"value": -40}
    noise_scale = {"value":0.4}
    pad_seconds = {"value":0.5}
    cl_num = {"value":0}
    lg_num = {"value":0}
    lgr_num = {"value":0.75}
    f0_predictor = {"value":"mp"}
    enhancer_adaptive_key = {"value":0}
    cr_threshold = {"value":0.5}
    k_step = {"value":100}
    use_spk_mix = {"value":False, "interactive":False}
    second_encoding = {"value":False}
    loudness_envelope_adjustment = {"value":0}
    _volume = { "minimum" : -1, "maximum" :1.5,"value":0,"step":0.1}
    _lang = {"value":"auto"}
    _gender = {"value":"女"}
    _rate = { "minimum" : -1, "maximum" :3,"value":0,"step":0.1}
    _text = "我是三月七"

    vc_infer(output_format,sid,input_audio,"tts",vc_transform,auto_f0,cluster_ratio,slice_db,noise_scale,pad_seconds,
             cl_num,lg_num,lgr_num, f0_predictor,enhancer_adaptive_key,cr_threshold,k_step,use_spk_mix, second_encoding,
             loudness_envelope_adjustment)

    vc_fn2(_text, _lang, _gender, _rate, _volume, sid, output_format, vc_transform,
           auto_f0, cluster_ratio, slice_db, noise_scale, pad_seconds, cl_num, lg_num,
           lgr_num, f0_predictor, enhancer_adaptive_key, cr_threshold, k_step, use_spk_mix,
           second_encoding, loudness_envelope_adjustment)
    main()
